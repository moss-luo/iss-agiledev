...
(function($) {
	...
	function renderUI(id, target,up) {
					...
					'<div class="plupload">' +
						'<div class="plupload_header">' +
							'<div class="plupload_header_content">' +
								...
								'<div class="plupload_header_text">' + _('Add files to the upload queue and click the start button.')+'('+up.settings.filters[0].extensions
	+')' + '</div>' +
							'</div>' +
						'</div>' +
		...
		if (settings) {
			this.each(function() {
				...
				function updateList() {
					...
					var _size = $(uploader.files).size() - 1;
					$.each(uploader.files, function(i, file) {
						...
						if(_size == i){
							if(file.name.length>50){
								$.fn.showMsg( {msg : _('Select files name over max')});
							}
						}
						fileList.append(
							'<li id="' + file.id + '">' +
								'<div class="plupload_file_name"><span title="'+file.name+'">' + file.name + '</span></div>' +
								'<div class="plupload_file_action"><a href="#" title="' + _('delete') + '"></a></div>' +
				...
				uploader.bind("Error", function(up, err) {
					...
					if (file) {
						...
						if (err.code == plupload.FILE_EXTENSION_ERROR) {
							alert(_("Error: Invalid file extension: ")+up.settings.filters[0].extensions);
						}
